...
function ellint_3(k, v, phi) {
    ...
    if (v > predicator)
        throw new InvalidArgument_1.InvalidArgument("Error on std.ellint_3(): must be v < (1 / sin^2(phi)) -> (v = " + v + ", 1 / sin^2(phi) = " + predicator + ").");
...
function _Post_process(func, k, phi, formula) {
    if (Math.abs(k) > 1)
        throw new InvalidArgument_1.InvalidArgument("Error on std." + func + "(): must be |k| <= 1 -> (k = " + k + ").");
