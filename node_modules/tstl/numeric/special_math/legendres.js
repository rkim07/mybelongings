...
function assoc_legendre(n, m, x) {
    ...
    if ((n = Math.floor(n)) < 0 || (m = Math.floor(m)) < 0)
        throw new InvalidArgument_1.InvalidArgument("Error on std.assoc_legendre(): both n and m must be unsigned integer -> (n = " + n + ", m = " + m + ").");
    else if (Math.abs(x) > 1)
        throw new InvalidArgument_1.InvalidArgument("Error on std.assoc_legendre(): must be |x| <= 1 -> (x = " + x + ").");
    // MEMORIZATION
    var matrix = [[1, x]];
    matrix.length = m + 1;
