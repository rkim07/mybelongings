...
function cyl_bessel_j(n, x) {
    ...
    if (x < 0 && Math.floor(n) !== n)
        throw new InvalidArgument_1.InvalidArgument("Error on std.cyl_bessel_j(): n must be integer when x is negative -> (n = " + n + ", x = " + x + ").");
    else if (x === 0 && n !== 0)
        throw new InvalidArgument_1.InvalidArgument("Error on std.cyl_bessel_j(): n must be zero when x is zero -> (n = " + n + ", x = " + x + ").");
    // COMPUTATION
    if (n === Math.floor(n))
        return _J_int(n, x);
    else
...
function cyl_neumann(v, x) {
    if (x <= 0)
        throw new InvalidArgument_1.InvalidArgument("Error on std.cyl_neumann(): x must be greater than zero -> (x = " + x + ").");
...
function cyl_bessel_i(n, x) {
    // VALIDATION
    if (x < 0 && Math.floor(n) !== n)
        throw new InvalidArgument_1.InvalidArgument("Error on std.cyl_bessel_i(): n must be integer when x is negative -> (n = " + n + ", x = " + x + ").");
    else if (x === 0 && n !== 0)
        throw new InvalidArgument_1.InvalidArgument("Error on std.cyl_bessel_i(): n must be zero when x is zero -> (n = " + n + ", x = " + x + ").");
    // COMPUTATION
    if (n === .5)
...
function cyl_bessel_k(n, x) {
    if (x <= 0)
        throw new InvalidArgument_1.InvalidArgument("Error on std.cyl_bessel_k(): requires x > 0 -> (x = " + x + ").");
